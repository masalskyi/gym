...
class CarRacerModel(MaslouRLModelDDPGContinuous):
    def build_critic_model(self):
        state_input = layers.Input(shape=self.input_shape)
        x = layers.Conv2D(filters=1, kernel_size=5, activation="relu", data_format='channels_first')(state_input)
        x = layers.Conv2D(filters=1, kernel_size=3, activation="relu", data_format='channels_first')(x)
        # x = layers.MaxPool2D(pool_size=2)(x)
